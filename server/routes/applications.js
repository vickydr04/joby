...
router.get('/', async function(req, res) {
  try {
    const entities = await repository.findAll({ parentId: req.params.jsId });
    res.json(entities);
  } catch (err) {
...
router.post('/', async function(req, res) {
  try {
    const entity = await repository.create({ ...req.body, parentId: req.params.jsId });
    res.json(entity);
  } catch (err) {
    res.status(400).json({ message: err.message });
  }
...
router.get('/:appId', async function(req, res) {
  try {
      const entities = await repository.findAll({ _id: req.params.appId, parentId: req.params.jsId });
...
router.patch('/:appId', async function(req, res) {
  try {
    const updateCount = await repository.update({ _id: req.params.appId, parentId: req.params.jsId }, req.body);
    if (updateCount === 0) {
      throw new Error(`Entity with id "${req.params.appId}" does not exist in parent entity with id "${req.params.jsId}"`);
    }
    res.end();
...
router.delete('/:appId', async function(req, res) {
  try {
    const deleteCount = await repository.delete({ _id: req.params.appId, parentId: req.params.jsId });
    if (deleteCount === 0) {
      throw new Error(`Entity with id "${req.params.appId}" does not exist in parent entity with id "${req.params.jsId}"`);
    }
    res.end();
